...
class ChatService:
    ...
    async def resolve_session(self, request: Request, response: Response, session_uuid_input: str | None = None) -> tuple[ChatSession, str]:
        session_uuid = (
            session_uuid_input or 
            request.cookies.get(self.SESSION_COOKIE_NAME) or 
            request.headers.get("X-Session-UUID")
        )
    ...
    async def chat(self, session: ChatSession, session_uuid: str, user_message: str) -> Dict[str, Any]:
